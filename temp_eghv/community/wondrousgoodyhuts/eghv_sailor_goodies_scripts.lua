...
-- (u_x)/ valkrana.moe
-- /////////////////////////////////
if g_iLoggingLevel > 1 then print("Loading component script EGHV_Sailor_Goodies_Scripts.lua . . ."); end
-- Support Functions
-- /////////////////////////////////
-- City plots.
local iCityRadius = 5;
function WGH_CityPlots(pCity)
	local tTempTable = {};
	if pCity ~= nil then
		...
		for dx = (iCityRadius * -1), iCityRadius do
			for dy = (iCityRadius * -1), iCityRadius do
				local plot = Map.GetPlotXYWithRangeCheck(iCityX, iCityY, dx, dy, iCityRadius);
...
for _, row in ipairs(DB.Query("SELECT DISTINCT FeatureType FROM Feature_Removes")) do
	table.insert(tValidFeatures, GameInfo.Features[row.FeatureType].Index);
end
function WGH_Resource_Tiles(tCities, repeatnum)
	local iCityRadius = 5;
	local tTempTable = {};
							...
							if repeatnum == 0 then
								if plot:GetFeatureType() > -1 then
									if plot:IsCity() then
										...
										for _, index in ipairs(tValidFeatures) do
...
-- /////////////////////////////////
-- // RANDOM RESOURCE
function WGH_Resource(playerID)
    ...
    if pCapital then
        ...
        repeat
            tValidTiles = WGH_Resource_Tiles(tCities, repeatnum);
            local rollcount = 0;
...
for _, row in ipairs(DB.Query("SELECT * FROM Units WHERE Domain != 'DOMAIN_AIR' AND ReligiousStrength = 0 AND TraitType NOT NULL AND CanRetreatWhenCaptured = 0 AND UnitType NOT LIKE 'UNIT_HERO%' AND UnitType NOT IN ('UNIT_BARBARIAN_RAIDER', 'UNIT_BARBARIAN_HORSEMAN')")) do
    ...
    table.insert(tUUs, {PrereqTech = row.PrereqTech, PrereqCivic = row.PrereqCivic, UnitType = row.UnitType, Domain = row.Domain, CivicEra = eraC, TechEra = eraT});
end

function WGH_Unit(playerID, iX, iY)
	...
	if pCap then
		...
		for dx = (pCapRadius * -1), pCapRadius do
			for dy = (pCapRadius * -1), pCapRadius do
				...
				if sPlotNearCap and ((sPlotNearCap:GetOwner() == player) or (sPlotNearCap:GetOwner() == -1)) then -- Is either neutral or belongs to player.
					if GameInfo.Terrains[sPlotNearCap:GetTerrainType()].TerrainType == "TERRAIN_COAST" and not sPlotNearCap:IsLake() then -- Coast and not lake.
...
for _, row in ipairs(DB.Query("SELECT * FROM Improvements WHERE RemoveOnEntry = 0 AND TraitType NOT NULL AND ImprovementType NOT IN ('IMPROVEMENT_BARBARIAN_CAMP', 'IMPROVEMENT_GOLF_COURSE', 'IMPROVEMENT_MEKEWAP', 'IMPROVEMENT_KAMPUNG', 'IMPROVEMENT_PAIRIDAEZA', 'IMPROVEMENT_POLDER', 'IMPROVEMENT_PYRAMID', 'IMPROVEMENT_FEITORIA') AND ImprovementType NOT IN (SELECT Value FROM ModifierArguments WHERE ModifierId IN (SELECT ModifierId FROM Modifiers WHERE ModifierType = 'MODIFIER_CITY_ADJUST_ALLOWED_IMPROVEMENT'))")) do
    table.insert(tUIs, {ImprovementType = row.ImprovementType, PrereqCivic = row.PrereqCivic, PrereqTech = row.PrereqTech});
end

function WGH_Improvement(playerID)
    local player            = Players[playerID];
    local playerEra 		= player:GetEras():GetEra();
...
-- // SIGHT
function WGH_Sight(_1, _2, _3, _4, unitAbility)
...
-- // FORMATION
function WGH_Formation(_1, _2, _3, unit, unitAbility)
...
function WGH_Wonder(playerID, iX, iY, unit)
...
-- // CITY-STATE
function WGH_CityState(playerID)
	local player 			= Players[playerID];
...
-- // PRODUCTION
function WGH_Production(playerID)
...
-- // TELEPORT
function WGH_Teleport(playerID, iX, iY, unit)
...
local tHutties = {
	[1999084244]   = {Name = "RESOURCE",     Func = WGH_Resource}, 
	[1625079733]   = {Name = "UNIT",         Func = WGH_Unit}, 
	[-2106477136]  = {Name = "IMPROVEMENT",  Func = WGH_Improvement}, 
	[-1461129027]  = {Name = "SIGHT",        Func = WGH_Sight}, 
	[260349556]    = {Name = "FORMATION",    Func = WGH_Formation}, 
	[1157765560]   = {Name = "POLICY",       Func = WGH_Policy}, 
	[-2028412409]  = {Name = "WONDER",       Func = WGH_Wonder}, 
	[1379672131]   = {Name = "CITYSTATE",    Func = WGH_CityState}, 
	[-2099557375]  = {Name = "SPY",          Func = WGH_Spy}, 
	[913875234]    = {Name = "PRODUCTION",   Func = WGH_Production}, 
	[-861896920]   = {Name = "TELEPORT",     Func = WGH_Teleport}
};

function Sailor_WGH(self, playerID, unitID, goodytype, subgoodytype) 
	if self.TypeHash ~= -304388911 then return; end -- Wondrous only.
	if (self.PlayerID == -1) or (self.UnitID == -1) then return; end -- Catch for nil. Thanks, zzragnar0kzz.
	...
	local unit				= self.Player:GetUnits():FindID(self.UnitID);
	local unitAbility		= unit:GetAbility();
	local iX, iY			= unit:GetX(), unit:GetY();
	local bSuccess = tHutties[self.RewardHash].Func(self.PlayerID, iX, iY, unit, unitAbility);
	print(string.format("[i]: Wondrous Goody Activated: %d %s", self.RewardHash, tHutties[self.RewardHash].Name));
	if not bSuccess then -- Random unique as fallback if reward can't be granted for any reason.
		print(string.format("[-]: Wondrous Goody 'FAILED': %d %s; resorting to fallback", self.RewardHash, tHutties[self.RewardHash].Name));
		bSuccess = WGH_Unit(self.PlayerID, iX, iY);
		print("[i]: Wondrous Goody Activated: 1625079733 UNIT");
		if self.IsHuman then -- Notify unique unit on failure.
			Game.AddWorldViewText(self.PlayerID, Locale.Lookup("LOC_WGH_FLOAT_FALLBACK"), iX, iY, 0);
		end
	end
	return true;
end
-- Events.GoodyHutReward.Add(Sailor_WGH);
